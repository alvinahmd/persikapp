<template>
  <div>
    <div v-if="showReset" class="fixed top-0 left-0 flex justify-center items-end w-screen h-full">
      ...
      <div class="w-full max-w-screen-sm bg-white max-h-screen" style="z-index: 1;">
        ...
        <div class="container px-5 space-y-10 py-3">
          <p>
            Masukkan email yang anda gunakan untuk registrasi.Kode verifikasi akan dikirimkan ke email anda.
          </p>
          <div class="w-full label border-b-4 text-base focus-within:border-purple-400 duration-500 transform border-gray-500">
            Email
            <input type="text" placeholder="Masukan Email Kamu" class=" pt-2 bg-transparent w-full focus:outline-none placeholder-gray-300">
          </div>
          <button class="rounded-lg w-full text-white text-base font-bold py-3 bg-purple-900 hover:bg-purple-700 duration-500" @click="showOtp=true">
            Kirim
          </button>
        </div>
      </div>
    </div>
    <verifikasiPage :show-otp="showOtp" />
  </div>
</template>
<script>
export default {
